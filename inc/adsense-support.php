...
function google_append_color(&$url, $param) {
	...
	google_append_url ( $url, $param, $color_array [$google_dt % count ( $color_array )] );
}
...
function google_mobile_ad( $id, $slot ) {

	$GLOBALS ['google'] ['client'] = $id;
	$GLOBALS ['google'] ['slotname'] = $slot;
	...
	if ($google_ad_handle) {
		while ( ! feof ( $google_ad_handle ) ) {
			$gmobileAd .= fread ( $google_ad_handle, 8192 );
		}
